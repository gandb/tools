...
let debugCounter = 1;
export async function follout(args) {
    const WORDS_INDEX = 3;
    if (!args[WORDS_INDEX] || args[WORDS_INDEX] < 3) {
        console.error("A quantidade de palavras deve ser maior que 3 \n");
        return;
    }
    const wordsBaseName = (args[WORDS_INDEX].split(",")).map((value) => value.toUpperCase());
    if (wordsBaseName.length < 4) {
        console.error("A quantidade de palavras deve ser maior que 3 \n");
        return;
    }
    const size = wordsBaseName[0].length;
    const errorSize = wordsBaseName.filter((word) => {
        if (word.length != size) {
    ...
    words.forEach((word, i) => {
        wordsBaseName.forEach((wordBaseValue, j) => {
            ...
            const candidate = new Word(wordBaseValue);
    ...
    while (notFound) {
        ...
        let nextChoise = words.reduce((previous, current) => {
        ...
        let conputerReturn = await askQuestion(`A sua próxima escolha deve ser : [${nextChoise.baseWord}] Qual o retorno do computador? (Digite apenas "OK" se a senha estiver correta)`);
        const lastChoise = nextChoise;
...
const DEBUG_WORD = "DEBUG"; //DEBUG_OFF,SWISS,CALLS,DOORS,NESTS; <=testes para o cenário de uso
function removeNonComplient(words, lastWordCandidate, computerReturn) {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        for (let j = 0; j < words.length; j++) {
            ...
            const removedIntoThisBlock = !wordCandidate.canBe || !word.canBe;
